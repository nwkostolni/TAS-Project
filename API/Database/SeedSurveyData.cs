...
namespace TAS_Project.Database
{
    public class SeedSurveyData : ISeedData
    {
        public void SeedData(){ //Method for Reseeding all the Survey Data
            ...
            cmd.CommandText=@"CREATE TABLE Survey(SurveyId INTEGER PRIMARY KEY, Cycle TEXT, DateDue TEXT, BeenCompleted BIT, DateCompleted TEXT, ReviewerEmpId INTEGER, SubjectEmpId INTEGER, FOREIGN KEY(ReviewerEmpId) REFERENCES Employee(EmployeeId), FOREIGN KEY(SubjectEmpId) REFERENCES Employee(EmployeeId))"; //Creates a new table
            cmd.ExecuteNonQuery();
